...
public class SignupController {
  ...
  public String signUpPost(@RequestParam(name = "planId", required = true) int planId,
    ...
    // There are certain info that the user doesn't set, such as profile image URL, Stripe customer
    // id,
    // plans and roles
    ...
    if (planId == PlansEnum.BASIC.getId()) {
      ...
      // If the user has selected the pro account, creates the Stripe customer to store the stripe
      // customer id in
      // the db
      Map<String, Object> stripeTokenParams =
          StripeUtils.extractTokenParamsFromSignupPayload(payload);

      Map<String, Object> customerParams = new HashMap<String, Object>();
      customerParams.put("description",
          "DevOps Buddy customer. Username: " + payload.getUsername());
  ...
  /**
   * Checks if the username/email are duplicates and sets error flags in the model. Side effect: the
   * method might set attributes on Model
   **/
